  ...
  return (
    <Card className="bg-[#0F172A] border-[#1E293B] overflow-hidden">
      <CardHeader className="px-4 py-3 border-b border-[#1E293B] flex flex-row items-center justify-between space-y-0">
        <span className="font-mono text-[11px] font-semibold uppercase tracking-widest text-[#94A3B8]">Commodities</span>
            ...
            <div key={group}>
              ...
              {!collapsed[group] && rows.map((a, i) => (
                ...
                >
                  ...
                  <span className="font-mono text-xs text-[#F8FAFC] w-24 text-right">{fmtPrice(a.close, a.unit)}</span>
                  <span className={`font-mono text-xs w-16 text-right ${pctColor(a.change_pct)}`}>{fmtPct(a.change_pct)}</span>
                </div>
              ))}
            </div>
          )
